...
export class HeartbeatManager {
  ...
  public build(manager: ClusterManager) {
    manager["heartbeat"] = this;
    this.manager = manager;
    this.start();
  }

  public start() {
    this.manager?.on("clusterCreate", (cluster: Cluster) => {
      cluster.on("ready", () => {
  ...
  public stop(cluster: Cluster, reason: string) {
    ...
    } else this.manager?._debug("[Heartbeat_MISSING] Respawn Rejected | Max Restarts of Cluster Reached", cluster.id);
  }
  ...
  public start() {
    return (this.interval = setInterval(() => {
      ...
      if (this.heartbeats.size > this.options.maxMissedHeartbeats) {
        ...
        this.instance.emit("heartbeatMissed", this.heartbeats.size);
      }
