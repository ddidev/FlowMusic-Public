...
import {
  LavalinkPlayer, PlayerEvent, PlayerEvents, PlayerUpdateInfo, RoutePlanner, Session, Structure,
...
export class Node {
  ...
  public destroy(): void {
    ...
    if (players.size) players.forEach(p => {
      this.manager.emit("playerDisconnected", p);
      p.destroy();
    });
